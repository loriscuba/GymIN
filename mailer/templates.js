// Template email GymIN — HTML table-based (rendono bene su tutti i client).
// Un layout base + un template per ogni tipo di comunicazione.
...
// Layout comune: intestazione GymIN, corpo, footer con disiscrizione (GDPR).
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #e6e8ee">
        <tr><td style="background:${ACCENT};padding:20px 28px">
          <span style="color:#fff;font-size:22px;font-weight:bold;letter-spacing:-.5px">GymIN</span>
        </td></tr>
        <tr><td style="padding:28px">
          <h1 style="margin:0 0 14px;font-size:20px;color:${INK}">${titolo}</h1>
          <div style="font-size:15px;line-height:1.6;color:#333">${corpo}</div>
          ${cta ? `<div style="margin:24px 0 6px">
            <a href="${cta.href}" style="background:${ACCENT};color:#fff;text-decoration:none;font-weight:bold;font-size:15px;padding:12px 22px;border-radius:10px;display:inline-block">${cta.label}</a>
          </div>` : ''}
        </td></tr>
        <tr><td style="padding:18px 28px;border-top:1px solid #e6e8ee">
          <p style="margin:0;font-size:12px;color:${MUTED};line-height:1.5">
            GymIN · Via dello Sport 12, Milano · +39 02 000 0000<br>
            Ricevi questa email come iscritto della palestra.
            <a href="{{UNSUBSCRIBE}}" style="color:${MUTED}">Disiscriviti</a>.
          </p>
        </td></tr>
      </table>
    </td></tr>
...
export const templates = {
  ...
  ricevuta({ socio, piano, importo, scadenza }) {
    return {
  ...
  rinnovo({ socio, piano, scadenza, giorni }) {
    return {
